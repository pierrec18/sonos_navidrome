...
import { store, LinkRecord, AuthType } from "./store.js";
...
function allowedRedirect(uri: string | undefined) {
  if (!uri) return false;
...
router.post("/oauth/authorize", express.urlencoded({ extended: true }), async (req, res) => {
  ...
  if (!allowedRedirect(redirect_uri)) return res.status(400).send("redirect_uri not allowed");
  if (!baseURL || !user) return res.status(400).send("Missing baseURL or user");
  let auth: AuthType;
  if (password) auth = { kind: "password", password };
  else if (token && salt) auth = { kind: "token", token, salt };
  else return res.status(400).send("Provide password OR token+salt");
...
router.post("/oauth/token", express.urlencoded({ extended: true }), (req, res) => {
  ...
  if (!code || typeof code !== "string" || !code.includes(".")) return res.status(400).json({ error: "invalid_request" });
  if (process.env.OAUTH_CLIENT_ID && client_id && process.env.OAUTH_CLIENT_ID !== client_id) return res.status(400).json({ error: "invalid_client" });
  if (process.env.OAUTH_CLIENT_SECRET && client_secret && process.env.OAUTH_CLIENT_SECRET !== client_secret) return res.status(400).json({ error: "invalid_client" });
...
router.post("/unlink", express.json(), (req, res) => {
  ...
  if (!access_token) return res.status(400).json({ ok: false, error: "missing token" });
